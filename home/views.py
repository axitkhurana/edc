...
def contact(request):
    ...
    try:
        ...
        add_secys = Member.objects.filter(rank=ranks['add_secy'])
        web_masters = Member.objects.filter(rank=ranks['web_master'])
    except ObjectDoesNotExist:
        pass

    return render_to_response('contact.html', {'list': menu, 'members': members, 'secy': secy, 'add_secys': add_secys, 'web_masters': web_masters}, context_instance=RequestContext(request))
